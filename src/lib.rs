...
pub fn chacha20_poly1305_encrypt(data: &Vec<u8>, additional_data: &Vec<u8>, key: &Vec<u8>, nonce: &Vec<u8>) -> Result<(Vec<u8>, [u8; 16]), Error> {
...
pub fn chacha20_poly1305_decrypt(ciphertext: &Vec<u8>, additional_data: &Vec<u8>, mac: &[u8; 16], key: &Vec<u8>, nonce: &Vec<u8>) -> Result<Vec<u8>, Error> {
...
pub fn chacha20_poly1305_ietf_encrypt(data: &Vec<u8>, additional_data: &Vec<u8>, key: &Vec<u8>, nonce: &Vec<u8>) -> Result<(Vec<u8>, [u8; 16]), Error> {
    let mut ciphertext = vec![0; data.len()];
    let mut mac: [u8; 16] = [0; 16];
...
pub fn chacha20_poly1305_ietf_decrypt(ciphertext: &Vec<u8>, additional_data: &Vec<u8>, mac: &[u8; 16], key: &Vec<u8>, nonce: &Vec<u8>) -> Result<Vec<u8>, Error> {
    let mut plaintext = vec![0; ciphertext.len()];
...
mod test_monocypher_binding{
    ...
    fn test_encryption_decryption(){
        let data = vec![0x10, 0x20, 0x30, 0x40];
        let key = vec![0x79,0x12,0xf9,0xf5,0xc6,0x82,0x0b,0x4c,0x00,0x99,0xb5,0x92,0x6c,0xb0,0xb0,0x0b,0x20,0x67,0xc1,0xda,0x3c,0xa1,0x45,0x89,0x81,0xdc,0x69,0xc8,0x39,0xb8,0xae,0xa5];
        let nonce = vec![0x79,0x12,0xf9,0xf5,0xc6,0x82,0x0b,0x4c,0x00,0x99,0xb5,0x92,0x6c,0xb0,0xb0,0x0b];
    ...
    fn test_ietf_encryption_decryption(){
        let data = vec![0x10, 0x20, 0x30, 0x40];
        let key = vec![0x79,0x12,0xf9,0xf5,0xc6,0x82,0x0b,0x4c,0x00,0x99,0xb5,0x92,0x6c,0xb0,0xb0,0x0b,0x20,0x67,0xc1,0xda,0x3c,0xa1,0x45,0x89,0x81,0xdc,0x69,0xc8,0x39,0xb8,0xae,0xa5];
        let nonce = vec![0x79,0x12,0xf9,0xf5,0xc6,0x82,0x0b,0x4c,0x00,0x99,0xb5,0x92];
        let (ciphertext, mac) = chacha20_poly1305_encrypt(&data,  &Vec::<u8>::new(), &key, &nonce).unwrap();
        let plaintext = chacha20_poly1305_decrypt(&ciphertext, &Vec::<u8>::new(), &mac, &key, &nonce).unwrap();
        assert_eq!(data, plaintext);
    }
    ...
    fn test_ietf_decryption_rfc(){
        ...
        let key: Vec<u8> = vec![0x1c,0x92,0x40,0xa5,0xeb,0x55,0xd3,0x8a,0xf3,0x33,0x88,0x86,0x04,0xf6,0xb5,0xf0,0x47,0x39,0x17,0xc1,0x40,0x2b,0x80,0x09,0x9d,0xca,0x5c,0xbc,0x20,0x70,0x75,0xc0];
        let nonce: Vec<u8> = vec![0x00,0x00,0x00,0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08];
